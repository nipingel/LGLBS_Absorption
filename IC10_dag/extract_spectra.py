...
def extract_emission_spectrum(em_cube_name, ra, dec):
    ...
    # 10/28/2024 - use_dask implementation to save up memory space
    cube = 	SpectralCube.read(em_cube_name, use_dask=True)
    central_c = SkyCoord(ra, dec, frame='fk5', unit=u.deg)
    radius = 5 * u.arcmin  # 5' diameter
    region = CircleSkyRegion(center=central_c, radius=radius)
    em_cube = cube.subcube_from_regions([region])
    header = em_cube.header
    warnings.filterwarnings("ignore", category=UserWarning, module="spectral_cube.wcs_utils")
    del cube  # This will mark the object for deletion
    beam_pa = header['BPA']  # Beam position angle in degrees
    beam_maj = header['BMAJ'] * 3600  # in arcseconds
    beam_min = header['BMIN'] * 3600  # in arcseconds
    pix_size = np.abs(header['CDELT1']) * 3600  # in arcseconds
    pix_area = (header['CDELT1'])**2*3600**2
# Calculate beam area and size in pixels
    beam_area_pixels = (beam_maj * beam_min * np.pi) / (4 * pix_area)
    beam_size_pixels = np.abs(np.sqrt(beam_area_pixels) / pix_size)

    print(f"Beam size in pixels: {np.abs(beam_size_pixels)}")
    R_out = 2 * beam_size_pixels
    R_in = beam_size_pixels

    em_wcs = em_cube.wcs
    central_c = SkyCoord(ra, dec, frame='fk5', unit=u.deg)  # Assuming `ra` and `dec` are defined
    c_pix = central_c.to_pixel(em_wcs)

    vel_axis = em_cube.spectral_axis.value / 1000.0  # Convert spectral axis to km/s
    ...
    radial_mask[r_pixs < R_out] = 1  # Set pixels within R_out to 1
    radial_mask[r_pixs < R_in] = 0   # Mask inner pixels within R_in

    # Extend the spatial mask along the entire spectral axis
    ...
    noise_per_channel = np.zeros(em_cube_Tb.shape[0])
    for i in range(em_cube_Tb.shape[0]):
        mask_area_pixels = np.sum(radial_mask) # / 3600
        n_beams_in_mask = mask_area_pixels / beam_area_pixels
        sigma_T =sigma_rms / np.sqrt(n_beams_in_mask)
        noise_per_channel[i] = sigma_T
    s_err = noise_per_channel
