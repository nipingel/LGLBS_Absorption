...
def source_find_mom0(cubelet_name):
	## make integrated intensity image
	hdu = fits.open(cubelet_name)
	data = hdu[0].data
	mom0_image = np.sum(data, axis = 0)
	fits.writeto(cubelet_name.replace('.fits', '.mom0.fits'), data, header = hdu[0].header)
	hdu.close()

	## run source-finding
	os.system('aegean %s --beam %s %s %s --out aegean_catalog.csv' % (cubelet_name.replace('.fits', '.mom0.fits'), bmaj, bmin, bpa))
	return mom0_image
...
def parse_csv(f):
	...
	return ra_column, dec_column, a_column, b_column, pa_column
...
def extract_mean_pixel_spectrum(mask, cubelet_name, vel_axis, bmaj, bmin, bpa, ra, dec, a, b, pa):
	...
	pix_size = (cubelet_hdu[0].header['CDELT1'])*3600
	pix_area = (cubelet_hdu[0].header['CDELT1'])**2*3600**2
	hdu.close()
...
def extract_emission_spectrum(em_cube_name, ra, dec, beam_maj, pix_size):
	...
	central_c = SkyCoord(ra, dec, frame = 'fk5')
	c_pix.to_pixel(em_wcs)
	hdu.close()
...
def compute_mean_em_spectrum(em_cube_name):
	...
	mean_em_spectrum = np.nanmean(cutout_cube, axis = (1, 2))*6.07e5/(beam_maj*beam_min) ## Jy/beam -> Tb
...
def generate_plot(vel_axis, abs_spectrum, em_spectrum, three_sig_env, one_sig_env, rms, detection_sort, ra, dec):
	...
	ax1.plot(vel_axis, abs_spectrum, color = tableau20[2], linestyle = '--')
	...
	## do plotting
	ax2.plot(vel_axis, em_spectrum*(6.07e5/(5.2*7)))
	ax2.fill_between(vel_axis, em_spectrum*(6.07e5/(5.2*7))-rms, em_spectrum*(6.07e5/(5.2*7))+rms, color = 'grey', alpha = 0.5)
...
def compute_mean_spin_temp(em_profile, rms, abs_profile, abs_profile_err, dv):
...
def main():
	...
	mom0_image = source_find_mom0(cubelet_name)

	## extract absorption spectrum based source catalog
	## get ellipse parameters
	comp_ra, comp_dec, comp_a, comp_b, comp_pa = parse_csv('aegean_catalog.csv')
	for i in range(len(comp_ra)):
		HI_abs_spectrum = extract_mean_pixel_spectrum(mask, cubelet_name, abs_vel_axis, comp_ra[i], comp_dec[i], comp_a[i], comp_b[i], comp_pa[i])

		## extract emission spectrum 
		em_spectrum = extract_emission_spectrum(combined_name, beam_maj_arcsec, pix_size)
		...
		abs_noise_env = sigma_cont*((tsys+apeff*(mean_em_spectrum*(6.07e5/(5.2*7))))/tsys)
		...
		## generate plot
		generate_plot(abs_vel_axis, HI_abs_spectrum, em_spectrum_interp, three_sig_env, abs_noise_env, rms_K, detection_sort, comp_ra[i], comp_dec[i])
		...
		for l in detection_sort:
			if len(l) > 2:
				...
				spin_temp, spin_temp_err = compute_spin_temperature(em_spectrum_interp, em_profile_err, abs_profile, abs_profile_err, dv)
				spin_temp_list.append(spin_temp)
				spin_temp_err_list.append(spin_temp_err)
		## save out results
		pickle_results(abs_vel_axis, HI_abs_spectrum, em_spectrum_interp, three_sig_env, abs_noise_env, rms_K, detection_sort, comp_ra[i], comp_dec[i], spin_temp_list, spin_temp_err_list)
